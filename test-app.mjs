...
assert('3.2 dual-channel table', await page.evaluate(() => document.querySelector('.notes')?.textContent.includes('dual channel')));
assert('3.2 installation procedure', await page.evaluate(() => document.querySelector('.notes')?.textContent.includes('retention clips')));
await page.click('button.sub-nav-btn:has-text("3.3")');
assert('3.3 NVMe trap content', await page.evaluate(() => {
  const t = document.querySelector('.notes')?.textContent || '';
  return t.includes('M.2 is a shape') && t.includes('RAID');
}));
await page.click('button.sub-nav-btn:has-text("3.4")');
assert('3.4 PCIe generations table', await page.evaluate(() => document.querySelector('.notes')?.textContent.includes('PCIe 4.0')));
...
assert('4.1 shared responsibility table', await page.evaluate(() => {
  const t = document.querySelector('.notes')?.textContent || '';
  return t.includes('Shared responsibility') && t.includes('Guest OS');
}));
assert('4.1 deployment pick-list', await page.evaluate(() => document.querySelector('.notes')?.textContent.includes('DaaS')));
...
assert('5.1 scenario pick-list', await page.evaluate(() => document.querySelector('.notes')?.textContent.includes('Scenario cue')));
await page.click('button.sub-nav-btn:has-text("5.2")');
assert('5.2 symptom map table', await page.evaluate(() => document.querySelector('.notes')?.textContent.includes('Memory diagnostic')));
await page.click('button:has-text("All 5.0 topics")');
await page.click('text=5.3 Troubleshoot');
assert('5.3 RAID scenario pick-list', await page.evaluate(() => document.querySelector('.notes')?.textContent.includes('Do not initialise')));
await page.click('button:has-text("All 5.0 topics")');
await page.click('text=5.7 Troubleshoot');
assert('5.7 ping ladder content', await page.evaluate(() => {
  const t = document.querySelector('.notes')?.textContent || '';
